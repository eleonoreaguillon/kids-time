...
function exportChildAllDays(project: Project, child: Child) {
  ...
  const childTable = (dateStr: string, day: ShootingDay) => {
    ...
    const workOver = stats ? Math.max(0, stats.workMin - maxWork) : 0;
    const ampOver = stats ? Math.max(0, stats.amplitudeMin - maxAmp) : 0;
    const bStr = stats?.breakSlots.map((b) => `${formatTime(b.start)}-${formatTime(b.end)} (${formatMinutes(b.durationMin)}${b.valid ? "" : " ⚠"})`).join(", ") || "--";
    ...
    return `<tr>
      ...
      <td>${stats ? formatMinutes(stats.amplitudeMin) : "--"} / ${formatMinutes(maxAmp)}</td>
      ...
      <td style="font-size:8px">${bStr}</td>
    </tr>`;
  };
  ...
  <table><thead><tr>
    <th>Date</th><th>Période</th><th>Début</th><th>Fin</th><th>Amplitude</th><th>Travail / Max</th><th>Pauses valides</th><th>Plages de pauses</th>
  </tr></thead><tbody>`;
...
// Fix #6: PDF global recap — one big table
function exportProjectGlobalPDF(project: Project) {
  const sortedDates = Object.keys(project.shootingDays).sort();
  if (sortedDates.length === 0) { alert("Aucune journée de tournage dans ce projet."); return; }
  let html = `<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
  <style>body{font-family:Arial,sans-serif;font-size:9px;padding:12px}
  ...
  h1{font-size:15px}h2{font-size:11px;color:#444;font-weight:normal;margin-bottom:10px}
  table{width:100%;border-collapse:collapse;margin-bottom:16px;font-size:8px}
  th{background:#1e3a5f;color:white;padding:4px 5px;text-align:left}
  ...
  <table><thead><tr>
    <th>Date</th><th>Nom Prénom</th><th>Statut</th><th>Période</th><th>Début</th><th>Fin</th><th>Travail / Max</th><th>Amplitude / Max</th><th>Pauses val.</th>
  </tr></thead><tbody>`;
  for (const dateStr of sortedDates) {
    const day = project.shootingDays[dateStr];
    const dateLabel = new Date(dateStr + "T12:00:00").toLocaleDateString("fr-FR", { day: "numeric", month: "short" });
    for (const childId of day.child_ids || []) {
      const child = project.children.find(c => c.id === childId); if (!child) continue;
      const session = day.sessions?.[childId];
      const vacation = isVacation(child, dateStr);
      ...
      html += `<tr>
        ...
        <td>${child.role ? ROLE_LABELS[child.role] : "—"}</td>
        <td>${vacation ? "🌴 Vac." : "🏫 Scol."}</td>
        <td>${session?.start_time ? formatTime(session.start_time) : "--"}</td>
        <td>${session?.end_time ? formatTime(session.end_time) : "--"}</td>
        <td><span class="${workOver > 0 ? "over" : "ok"}">${stats ? formatMinutes(stats.workMin) : "--"} / ${formatMinutes(maxWork)}</span></td>
        <td><span class="${ampOver > 0 ? "over" : "ok"}">${stats ? formatMinutes(stats.amplitudeMin) : "--"} / ${formatMinutes(maxAmp)}</span></td>
        <td>${stats ? formatMinutes(stats.validBreakMin) : "--"}</td>
      </tr>`;
    }
  }
  html += `</tbody></table><div class="footer">Généré par KidsTime · Éléonore Aguillon · ACMA Fiction</div></body></html>`;
  const w = window.open("", "_blank"); if (w) { w.document.write(html); w.document.close(); setTimeout(() => w.print(), 500); }
}
...
function MainApp({ session, onSignOut }: { session: any; onSignOut: () => void }) {
  ...
  async function updateChild(id: string, data: { fullName: string; dob: string; vacationPeriods: VacationPeriod[]; role: ChildRole | null }) {
    ...
    const { error } = await supabase.from("children").update({ first_name: firstName, last_name: lastName, dob: data.dob, vacation_periods: data.vacationPeriods || [], child_role: data.role ?? null }).eq("id", id);
          ...
          <div className="space-y-3">
            ...
            {projects.map(p => (
              <div key={p.id} className="flex items-center gap-3 bg-slate-900/50 border border-slate-700 rounded-xl px-4 py-4 active:bg-slate-800 transition-colors cursor-pointer" onClick={() => onOpen(p.id)}>
                ...
                <button onClick={e => { e.stopPropagation(); onDelete(p.id); }} className="text-slate-600 hover:text-red-400 text-xl w-10 h-10 flex items-center justify-center">✕</button>
              </div>
            ))}
          </div>
        }
...
function ChildFormModal({ child, onSave, onClose }: { child: Child | null; onSave: (d: any) => void; onClose: () => void }) {
  ...
  function handleSave() {
    ...
    setError(""); onSave({ fullName: fullName.trim(), dob, vacationPeriods, role });
  }
...
function TimeActionModal({ type, childCount, dateStr, onConfirm, onClose }: { type: string; childCount: number; dateStr: string; onConfirm: (t: string) => void; onClose: () => void }) {
  ...
  const labels: Record<string, string> = { start: "Démarrer", pause: "Mise en pause", resume: "Reprise", end: "Fin de journée" };
  return (
...
function ChildCard({ child, session, stats, maxWork, breakAfter, maxAmplitude, vacation, isSelected, onSelect, isExpanded, onToggleExpand, onStart, onCancelSession, onCancelLastEvent, onReopenSession, onEditEventTime, onEditStartTime, onEditEndTime, dateStr }: {
  ...
  const statusColor = session?.status === "working" ? "border-emerald-700" : session?.status === "paused" ? "border-amber-600" : session?.status === "done" ? "border-slate-600" : workCrit || ampCrit ? "border-red-700" : breakDue ? "border-amber-600" : "border-slate-700";
          ...
          {stats && (
            <>
              <div className="grid grid-cols-3 gap-2">
                {([{ l: "Travail", v: stats.workMin, max: maxWork, crit: workCrit }, { l: "Pauses val.", v: stats.validBreakMin, sub: `tot.${formatMinutes(stats.breakMin)}` }, { l: "Amplitude", v: stats.amplitudeMin, max: maxAmplitude, crit: ampCrit }] as any[]).map(({ l, v, max, sub, crit }) => (
