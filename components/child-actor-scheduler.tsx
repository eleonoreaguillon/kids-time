...
// Detect role — only match exact words to avoid false positives
...
function exportDayToPDF(project: Project, dateStr: string) {
  ...
  const childTable = (row: any) => {
    ...
    const workOver = stats ? Math.max(0, stats.workMin - maxWork) : 0;
    const ampOver  = stats ? Math.max(0, stats.amplitudeMin - maxAmp) : 0;
    const bStr = stats?.breakSlots.map((b: any) => `${formatTime(b.start)}-${formatTime(b.end)} (${formatMinutes(b.durationMin)}${b.valid ? "" : " ⚠"})`).join("<br>") || "--";
    return `<table><tr><th colspan="4">${child.first_name} ${child.last_name}${child.role ? ` — ${ROLE_LABELS[child.role as ChildRole]}` : ""} — ${getAge(child.dob)} ans (${band} ans) — ${vacation ? "Vacances" : "Scolaire"}</th></tr>
      ...
      <tr><td><b>Travail total</b><br>${stats ? formatMinutes(stats.workMin) : "--"}</td><td><b>Max travail</b><br>${formatMinutes(maxWork)}</td><td><b>Dépassement travail</b><br><span class="${workOver > 0 ? "over" : "ok"}">${workOver > 0 ? formatMinutes(workOver) : "OK"}</span></td><td><b>Dépassement amplitude</b><br><span class="${ampOver > 0 ? "over" : "ok"}">${ampOver > 0 ? formatMinutes(ampOver) : "OK"}</span></td></tr>
      <tr><td><b>Pauses totales</b><br>${stats ? formatMinutes(stats.breakMin) : "--"}</td><td><b>Pauses valides</b><br>${stats ? formatMinutes(stats.validBreakMin) : "--"}</td><td colspan="2"><b>Plages de pauses</b><br>${bStr}</td></tr></table>`;
  };
  const allRows = buildExportRows(project, dateStr);
  let html = `<html><head><meta charset="utf-8"><style>body{font-family:Arial,sans-serif;font-size:11px;color:#111;padding:20px}h1{font-size:18px;margin-bottom:4px}h2{font-size:13px;color:#444;margin-bottom:16px;font-weight:normal}h3{font-size:12px;color:#1e3a5f;border-bottom:2px solid #1e3a5f;padding-bottom:4px;margin:20px 0 10px}table{width:100%;border-collapse:collapse;margin-bottom:16px}th{background:#1e3a5f;color:white;padding:7px 8px;text-align:left;font-size:10px}td{padding:6px 8px;border-bottom:1px solid #e5e7eb;vertical-align:top}tr:nth-child(even) td{background:#f8fafc}.over{color:#dc2626;font-weight:bold}.ok{color:#16a34a}.footer{margin-top:30px;font-size:9px;color:#999;text-align:center}</style></head><body>
  <h1>KidsTime — Récapitulatif journée</h1><h2>${dateLabel} · ${project.name}</h2>`;
    ...
    <div className="min-h-screen bg-[#080d16] flex flex-col items-center justify-center px-4" style={{ fontFamily: "'DM Mono', monospace" }}>
      ...
      <div className="relative z-10 w-full max-w-sm bg-slate-900/70 border border-slate-700 rounded-2xl p-7 backdrop-blur">
        <div className="flex mb-6 bg-slate-800 rounded-xl p-1">
          {(["login", "signup"] as const).map(m => <button key={m} onClick={() => { setMode(m); setError(""); }} className={`flex-1 py-2 rounded-lg text-sm font-semibold transition-all ${mode === m ? "bg-blue-600 text-white" : "text-slate-400 hover:text-white"}`}>{m === "login" ? "Connexion" : "Créer un compte"}</button>)}
        </div>
        <form onSubmit={submit} className="space-y-4">
          ...
          <button type="submit" disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white py-3 rounded-xl font-bold text-sm transition-colors">{loading ? "Chargement…" : mode === "login" ? "Se connecter" : "Créer mon compte"}</button>
        </form>
      </div>
    </div>
  );
...
function MainApp({ session, onSignOut }: { session: any; onSignOut: () => void }) {
  ...
  // BUG #2 FIX: role is explicitly cast to ChildRole | null — never undefined
  async function addChild(child: { fullName: string; dob: string; vacationPeriods: VacationPeriod[]; role: ChildRole | null }) {
  ...
  // BUG #1 FIX: strip the `fullName` preview field before inserting into Supabase
  async function addChildren(children: { firstName: string; lastName: string; dob: string; vacationPeriods: VacationPeriod[]; role: ChildRole | null; fullName?: string }[]) {
    if (children.length === 0) return;
    // Strip any extra fields — only send what Supabase expects
  ...
  // BUG #2 FIX: role is explicitly ChildRole | null
  async function updateChild(id: string, data: { fullName: string; dob: string; vacationPeriods: VacationPeriod[]; role: ChildRole | null }) {
  ...
  async function cancelSession(dateStr: string, childId: string) {
    const day = activeProject!.shootingDays[dateStr]; if (!day) return;
    const sessions = { ...(day.sessions || {}) }; delete sessions[childId]; await updateDaySessions(dateStr, sessions);
  }
  ...
  async function cancelLastEvent(dateStr: string, childId: string) {
    ...
    let status: Session["status"] = "working"; if (lastEv?.type === "pause_start") status = "paused";
  ...
  async function endSessions(dateStr: string, childIds: string[], timeISO?: string) {
    ...
    for (const childId of childIds) {
      const s = sessions[childId]; if (!s?.start_time || s.status === "done") continue;
      const events = [...(s.events || [])]; if (s.status === "paused") events.push({ type: "pause_end", time: timeISO || nowISO() });
  ...
  async function reopenSession(dateStr: string, childId: string) {
    const day = activeProject!.shootingDays[dateStr]; if (!day) return;
    const sessions = { ...(day.sessions || {}) }; sessions[childId] = { ...sessions[childId], status: "working", end_time: undefined };
    await updateDaySessions(dateStr, sessions);
  }

  async function editEventTime(dateStr: string, childId: string, eventIndex: number, newTimeISO: string) {
    const day = activeProject!.shootingDays[dateStr]; if (!day) return;
    const sessions = { ...(day.sessions || {}) }; const s = { ...sessions[childId] }; const events = [...(s.events || [])];
    events[eventIndex] = { ...events[eventIndex], time: newTimeISO }; s.events = events; sessions[childId] = s;
    await updateDaySessions(dateStr, sessions);
  }

  async function editStartTime(dateStr: string, childId: string, newTimeISO: string) {
    const day = activeProject!.shootingDays[dateStr]; if (!day) return;
    const sessions = { ...(day.sessions || {}) }; sessions[childId] = { ...sessions[childId], start_time: newTimeISO };
    await updateDaySessions(dateStr, sessions);
  }

  async function editEndTime(dateStr: string, childId: string, newTimeISO: string) {
    const day = activeProject!.shootingDays[dateStr]; if (!day) return;
    const sessions = { ...(day.sessions || {}) }; sessions[childId] = { ...sessions[childId], end_time: newTimeISO };
    await updateDaySessions(dateStr, sessions);
  }
    ...
    <div className="min-h-screen bg-[#080d16] text-white" style={{ fontFamily: "'DM Mono', monospace" }}>
      ...
      <div className="relative max-w-2xl mx-auto px-6 py-14">
        <div className="flex items-start justify-between mb-12">
          ...
          <div className="text-right mt-2"><div className="text-xs text-slate-500 mb-1">{userEmail}</div><button onClick={onSignOut} className="text-xs text-slate-500 hover:text-red-400 transition-colors">Déconnexion</button></div>
        </div>
        <div className="bg-slate-900/60 border border-slate-700 rounded-2xl p-6 mb-8">
          ...
          <div className="flex gap-3">
            <input className="flex-1 bg-slate-800 border border-slate-600 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-blue-500 placeholder:text-slate-600" placeholder="Titre du film ou de la série…" value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === "Enter" && name.trim() && (onCreate(name.trim()), setName(""))} />
  ...
  return (
    <div className="min-h-screen bg-[#080d16] text-white" style={{ fontFamily: "'DM Mono', monospace" }}>
      ...
      <div className="border-b border-slate-800 px-6">
        <div className="flex gap-1 -mb-px">{tabs.map(t => <button key={t.id} onClick={() => setTab(t.id as any)} className={`px-4 py-3 text-sm transition-colors border-b-2 ${tab === t.id ? "border-blue-500 text-white" : "border-transparent text-slate-500 hover:text-slate-300"}`}>{t.label}</button>)}</div>
      </div>
...
function ChildrenTab({ project, onAdd, onEdit, onRemove, onImport }: { project: Project; onAdd: () => void; onEdit: (c: Child) => void; onRemove: (id: string) => void; onImport: (cs: any[]) => Promise<void> }) {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (file.name.endsWith(".csv") || file.name.endsWith(".txt")) {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const row: any = {}; headers.forEach((h, idx) => { row[h] = vals[idx] || ""; }); rows.push(row);
        }
      } else {
      ...
      // Detect name columns — be careful not to confuse "statut" column with name
      ...
      // "nom" alone is too ambiguous — only use if no other name column found
      ...
      // Statut column — use very specific candidates only, never "nom"
      ...
      const parsed = rows.map(r => {
        ...
        if (fullCol && String(r[fullCol] || "").trim()) {
          ...
          const fn = String(r[fnCol] || "").trim(); const ln = String(r[lnCol] || "").trim();
        ...
        // BUG #1 FIX: detectRole returns ChildRole | null (not undefined)
        ...
        return {
          ...
          role,
          // preview only — will be stripped before Supabase insert
          _preview: `${firstName} ${lastName}`.trim(),
        };
    ...
    } catch (err) { console.error(err); setImportMsg("❌ Erreur de lecture. Vérifiez le format du fichier."); }
    e.target.value = "";
  }
  ...
  async function confirmImport() {
    setImporting(true);
    try {
      // Strip _preview field before passing to onImport
      const clean = importPreview.map(({ _preview, ...rest }) => rest);
    ...
    } catch { setImportMsg("❌ Erreur lors de l'import. Réessayez."); }
    setImporting(false);
  }
  ...
  return (
    <div>
      ...
      <div className="bg-slate-900/40 border border-slate-700 rounded-xl p-4 mb-5">
        ...
        <div className="text-[10px] text-slate-500 mb-3">Colonnes : <span className="text-slate-300">Nom Prénom</span> · <span className="text-slate-300">Date de naissance</span> · <span className="text-slate-300">Statut</span> (optionnel)</div>
      ...
      {project.children.length > 0 && rolesPresent.length > 0 && (
        <div className="flex gap-1 mb-4 border-b border-slate-800">
          <button onClick={() => setRoleTab("all")} className={`px-4 py-2 text-sm transition-colors border-b-2 ${roleTab === "all" ? "border-blue-500 text-white" : "border-transparent text-slate-500 hover:text-slate-300"}`}>Tous ({project.children.length})</button>
          {rolesPresent.map(r => (
          ...
          {project.children.some(c => !c.role) && (
            <button onClick={() => setRoleTab("all")} className="px-4 py-2 text-sm transition-colors border-b-2 border-transparent text-slate-500 hover:text-slate-300">
            ...
            <div key={g.id} className="bg-slate-900/50 border border-slate-700 rounded-xl p-5">
              <div className="flex items-center justify-between mb-3">
                {editingName === g.id ? (
                  <div className="flex items-center gap-2 flex-1 mr-3">
                    <input value={nameValue} onChange={e => setNameValue(e.target.value)} className="flex-1 bg-slate-800 border border-blue-500 rounded-lg px-3 py-1.5 text-white text-sm focus:outline-none"
                ...
                <div className="flex gap-2">
                  <button onClick={() => setEditing(editing === g.id ? null : g.id)} className="text-slate-400 hover:text-white text-sm px-2 py-1 rounded border border-slate-600 hover:border-slate-400">{editing === g.id ? "✓ Fermer" : "👥 Membres"}</button>
              ...
              {editing === g.id && (
                <div className="mb-4 bg-slate-800/50 rounded-xl p-3">
                  ...
                  {project.children.length === 0 ? <div className="text-xs text-slate-500">Aucun enfant dans le projet</div> :
                    <div className="space-y-1">{project.children.map(c => {
                      ...
                      return (
                        <label key={c.id} className="flex items-center gap-3 cursor-pointer hover:bg-slate-700/50 px-2 py-1.5 rounded-lg">
                          ...
                          <div className="flex gap-1 flex-wrap">
                            ...
                            {otherGroups.map(gn => <span key={gn} className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] bg-slate-600/60 text-slate-300 border border-slate-500">👥 {gn}</span>)}
                          </div>
                        </label>
                      );
                    })}</div>
        ...
        <div key={rk} className="mb-6">
          ...
          <div className="space-y-2">{AGE_BANDS.map(band => (
            <div key={band} className="bg-slate-900/50 border border-slate-700 rounded-xl p-4">
              ...
              <div className="grid grid-cols-2 gap-4">{(["school", "vacation"] as const).map(p => (
                <div key={p}>
...
// BUG #2 FIX: role is ChildRole | null throughout — never undefined or ""
function ChildFormModal({ child, onSave, onClose }: { child: Child | null; onSave: (d: any) => void; onClose: () => void }) {
  ...
  function handleSave() {
    ...
    setError("");
    // role is null (not "") when not selected — Supabase handles null correctly
    onSave({ fullName: fullName.trim(), dob, vacationPeriods, role });
  }
  ...
  return (
    <Modal title={child ? "Modifier l'enfant" : "Ajouter un enfant"} onClose={onClose}>
      <div className="space-y-4">
        ...
        {dob && <div className="bg-blue-900/30 border border-blue-700/60 rounded-lg px-4 py-2 text-sm text-blue-300">{getAge(dob)} ans · Tranche DRIEETS : {getAgeBand(dob)} ans</div>}

        <div className="flex flex-col gap-1">
          <label className="text-[10px] text-slate-400 uppercase tracking-[0.15em] font-semibold">Statut <span className="text-slate-600 normal-case font-normal">(optionnel)</span></label>
          <div className="flex gap-2 flex-wrap">
            {/* null = non défini */}
            <button type="button" onClick={() => setRole(null)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${role === null ? "bg-slate-600 border-slate-500 text-white" : "bg-slate-800 border-slate-600 text-slate-400 hover:text-white"}`}>
              Non défini
            </button>
            {ALL_ROLES.map(r => (
              <button key={r} type="button" onClick={() => setRole(r)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors ${role === r ? ROLE_COLORS[r] : "bg-slate-800 border-slate-600 text-slate-400 hover:text-white"}`}>
                {ROLE_LABELS[r]}
              </button>
            ))}
          </div>
        ...
        <div>
          <label className="text-[10px] text-slate-400 uppercase tracking-wider block mb-2">Périodes de vacances <span className="text-slate-600 normal-case font-normal">(optionnel)</span></label>
          {vacationPeriods.map((p, i) => (
            <div key={i} className="flex items-center gap-2 mb-1 text-sm text-slate-300">
              <span>{p.start} → {p.end}</span>
              <button onClick={() => setVacationPeriods(v => v.filter((_, j) => j !== i))} className="text-red-400">✕</button>
            </div>
          ))}
          <div className="flex gap-2 items-end mt-2">
            ...
            <button onClick={() => { if (newVac.start && newVac.end) { setVacationPeriods(v => [...v, newVac]); setNewVac({ start: "", end: "" }); } }} className="bg-slate-700 hover:bg-slate-600 text-white px-3 rounded-lg h-9 text-sm">+</button>
          </div>
        </div>
...
function ShootingView({ project, dateStr, onBack, onStartSessions, onStartSession, onCancelSession, onApplyEvent, onCancelLastEvent, onEndSessions, onReopenSession, onToggleChild, onAddGroup, onRemoveGroup, onEditEventTime, onEditStartTime, onEditEndTime, onExportXLSX, onExportPDF }: {
  ...
  onCancelSession: (cid: string) => void; onApplyEvent: (cids: string[], type: "pause_start" | "pause_end", t?: string) => void;
      ...
      <div className="max-w-4xl mx-auto px-6 py-6">
            ...
            <div className="mt-3 bg-slate-900/60 border border-slate-700 rounded-xl p-5">
              {project.groups.length > 0 && (
                <div className="mb-4">
                  ...
                  <div className="space-y-2">{project.groups.map(g => {
                    ...
                    return (
                      <div key={g.id} className="flex items-center gap-2">
                        <button onClick={() => onAddGroup(g.id)} className={`text-sm px-3 py-1.5 rounded-lg border transition-colors ${groupInDay ? "bg-blue-900/50 border-blue-600 text-blue-300" : "bg-slate-800 border-slate-600 text-slate-300 hover:border-blue-600 hover:text-blue-300"}`}>
                          + {g.name} ({g.child_ids?.length || 0}){groupPartial && <span className="text-amber-400 text-xs ml-1">partiel</span>}
                        </button>
                        {(groupInDay || groupPartial) && <button onClick={() => onRemoveGroup(g.id)} className="text-xs text-red-400 hover:text-red-300 border border-red-800/60 hover:border-red-600 px-2 py-1.5 rounded-lg transition-colors">− Retirer</button>}
                      </div>
                    );
                  })}</div>
                </div>
        ...
        {childIds.length > 0 && rolesPresent.length > 0 && (
          <div className="flex gap-1 mb-5 border-b border-slate-800">
            <button onClick={() => setRoleTab("all")} className={`px-4 py-2 text-sm transition-colors border-b-2 ${roleTab === "all" ? "border-blue-500 text-white" : "border-transparent text-slate-500 hover:text-slate-300"}`}>Tous ({childrenInDay.length})</button>
            {rolesPresent.map(r => <button key={r} onClick={() => setRoleTab(r)} className={`px-4 py-2 text-sm transition-colors border-b-2 ${roleTab === r ? "border-blue-500 text-white" : "border-transparent text-slate-500 hover:text-slate-300"}`}>{ROLE_LABELS[r]} ({childrenInDay.filter(c => c.role === r).length})</button>)}
          </div>
        )}
        ...
        {childIds.length > 0 && (
          <div className="bg-slate-900/60 border border-slate-700 rounded-xl px-5 py-3 mb-5 flex flex-wrap items-center gap-3">
            ...
            {project.groups.map(g => <button key={g.id} onClick={() => setSelected(s => { const n = new Set(s); (g.child_ids || []).forEach(id => n.add(id)); return n; })} className="text-xs text-slate-400 hover:text-white border border-slate-700 hover:border-slate-500 px-2 py-0.5 rounded">{g.name}</button>)}
            <div className="flex-1" />
            {selected.size > 0 && <>
        ...
        {childIds.length === 0
          ...
          : filteredIds.length === 0
            ...
            : <div className="space-y-4">{filteredIds.map(id => {
  ...
  return (
    <Modal title={labels[type]} onClose={onClose}>
      <div className="space-y-5">
        <div className="bg-slate-800/60 rounded-xl px-4 py-3 text-sm text-slate-300">Action appliquée à <span className="text-white font-bold">{childCount} enfant(s)</span> sélectionné(s)</div>
        <div className="flex flex-col gap-1">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => onConfirm(timeStrToISO(dateStr, timeStr))} className="flex-1 py-2.5 rounded-xl font-bold text-sm bg-blue-600 hover:bg-blue-500 text-white transition-colors">Confirmer — {timeStr}</button>
        </div>
      </div>
    </Modal>
  );
  ...
  return (
    <div className={`rounded-2xl p-5 border transition-all ${isSelected ? "border-blue-500 bg-blue-950/30" : workCrit || ampCrit ? "border-red-700 bg-slate-900/60" : breakDue ? "border-amber-600 bg-slate-900/60" : "border-slate-700 bg-slate-900/50"}`}>
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="w-10 h-10 rounded-full bg-blue-900/60 flex items-center justify-center text-blue-300 font-bold text-sm flex-shrink-0">{child.first_name?.[0]}{child.last_name?.[0]}</div>
        <div className="flex-1">
        ...
        <div className="text-right text-[10px] text-slate-500">
          <div>Max : {formatMinutes(maxWork)}</div><div>Pause / {formatMinutes(breakAfter)}</div><div>Ampl. {formatMinutes(maxAmplitude)}</div>
        </div>
      </div>
      ...
      {stats && (
        <>
          ...
          <div className="flex flex-wrap gap-2">
            {session?.status !== "done" && events.length > 0 && <button onClick={onCancelLastEvent} className="text-xs text-amber-400 hover:text-amber-300 border border-amber-800/60 hover:border-amber-600 px-3 py-1.5 rounded-lg transition-colors">↩ Annuler dernière action</button>}
            {session?.start_time && session?.status !== "done" && <button onClick={onCancelSession} className="text-xs text-red-400 hover:text-red-300 border border-red-800/60 hover:border-red-600 px-3 py-1.5 rounded-lg transition-colors">🗑 Réinitialiser</button>}
            {session?.status === "done" && <button onClick={onReopenSession} className="text-xs text-blue-400 hover:text-blue-300 border border-blue-800/60 hover:border-blue-600 px-3 py-1.5 rounded-lg transition-colors">↩ Rouvrir la journée</button>}
          </div>
        </>
      )}
